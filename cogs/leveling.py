...
class Leveling(commands.Cog):
    ...
    async def sync_show_levels(self, ctx): #管理者がギルド全員のレベルを名前の横に表示(更新)する
        ...
        # サーバーに存在する「Lv10」「Lv20」などのロールをすべて自動抽出してリスト化する
        available_role_levels = [
            int(r.name[3:]) for r in ctx.guild.roles 
            if r.name.startswith("Lv.") and r.name[3:].isdigit() and int(r.name[3:]) % 10 == 0
        ]
        # 抽出したロールの中から最大値を上限として設定（ロールが無い場合は0）
        max_role_level = max(available_role_levels) if available_role_levels else 0
        ...
        for member in ctx.guild.members:
            ...
            try:
                ...
                target_role_level = min((level // 10) * 10, max_role_level)

                #全レベルロールを確認し、正しいものだけ残して他は消す（クレンジング）
                for r_level in available_role_levels:
